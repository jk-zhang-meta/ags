...
### `ags ls` 同一天之内改按启动时间排（v0.7.1）

- **两级排序：先按最后活动的日期降序，同一天里按启动时间降序。** 上一条把顺序整个
  交给了分钟级的最后活动时间，那件事在跨天的粒度上是对的，在一天之内是错的——那个
  值每被碰一次就变，同一天的几条会话每次 `ags ls` 的相对位置都不一样，眼睛得重新
  找。启动时间在一天之内是固定的。

  「哪天动过」也正好是找会话真正用得上的粒度：你记得的是"今天还在弄的"和"前天那
  条"，不是"17:42 那条在 17:05 那条前面"。

- 日期用**本地时区**的 `YYYYMMDD`（jq 的 `strflocaltime`），不是 `epoch/86400`——
  后者切的是 UTC 的午夜，在东八区就是早上八点，"今天早上七点动过"的会话会被算进
  昨天。

### `ags ls` 能只看一个 Agent（v0.8.0）

- **`ags ls codex` / `ags ls claude` 只列那一家的会话**，菜单里 `f` 循环同一个过滤：
  全部 → 第一个 Agent → … → 全部。可选项来自这一批里**真出现过**的 Agent，不是写
  死的两档——装了新 Agent 不改代码也能循环到它，只用 Codex 的机器上也不会多出一档
  空的。

- **过滤只改视图，不改短号。** 短号是这台机器给会话的坐标，`ags resume 3` 得永远指
  同一条。按当前这一屏重新编号的话，先 `ags ls codex` 再敲 `ags resume 3` 打开的就
  不是刚看的那条了。所以过滤那一刀落在发号之后。

- 两套名字都认：ags 的 provider slug（`claude-code`）和 AGS 启动它时用的名字
  （`claude`）。`-a` 会把存档和活会话并排列出来，而这两半各自只带一套名字。

- 名字打错（`ags ls cladue`）会把可选的名字一起报出来。打错和"这台机器真的没有会话"
  在屏幕上长得一模一样，而前者是每天都会犯的错。

### 描述这一列：人能自己写，模型改成认"最近在做什么"（v0.8.0）

四级回退（人写的 > Agent 自己起的名字 > 我们算的总结 > 原话第一行）本身没变，三处
各自都有毛病：

- **第一级此前根本没有入口。** 人写的描述排在最前面，却没有任何界面能写它——实测一
  台用了几个月的机器上 79 份注解，0 份有非空描述。现在菜单里 `n` 就地改名，留空交
  回自动命名。有这一条比把自动方案调到完美更要紧：自动的总会有失手，而失手的时候
  用户此前只能干看着。

- **总结改成偏"最近"。** digest 的采样从「前 3 条 + 后 2 条」改成「前 2 条 + 后 4
  条」，提示词也明说以靠后的为准、靠前的只用来看清是哪个项目。理由有两条：一是长
  会话会漂题，而这一列要回答的是"我上次停在哪儿"；二是偏头的采样**算不出新东西**
  ——开头那几条是冻住的，于是每次重算都在重新推导同一句话，还要为此付一次模型调用。

- **敷衍的答复不再入库。** 实测拿到过"暂无可概括内容"这种回答：它非空，于是在回退
  里压过了原话第一行，把一个还算能认的标题换成一句废话，而且指纹不变就再也不会重
  算。现在提示词约定"没有可概括的内容就只输出一个减号"，减号当空处理——判断留在有
  信息的那一侧，而不是在 bash 里维护一张中英文废话黑名单。

### `ags ls` 排序回到最后活动时间，同一分钟看启动时间（v0.7.2）

- **两级排序：先按最后活动时间降序（截到分钟），同一分钟里按启动时间降序。** 上一
  条按「最后活动的**日期**」分桶，代价是 ACTIVE 那一列在屏幕上不再单调：真机上一
  屏是 `14:06 / 02:23 / 14:07 / 00:44 / 13:53 …`——同一天的十条按启动时间排，看上去
  就是乱的，也没法用眼睛核对。列表要回答的第一个问题是"刚才在动的那条在哪"，那条
  必须在最上面。

- **主键截到分钟，正好是 ACTIVE 那一列显示的粒度（`%m-%d %H:%M`）。** 于是从上往下
  读，ACTIVE 一定不增，顺序完全能对着屏幕核对，而不是靠一个看不见的秒数。分钟在任
  何时区都是对齐的（`+05:30`、`+05:45` 也是整分钟偏移），这一刀不用管时区，之前为
  日期分桶而用的 `strflocaltime` 也就不需要了。

- **同一分钟里按启动时间降序不是凑数的兜底。** 一台真机上的 40 条会话里有 11 组最
  后活动时间显示成同一分钟，一组多达 3 条——Codex 同一批被碰过的会话会拿到几乎一
  样的 `last_active_at`。没有第二个键，这几条的相对位置就是排序碰巧给出来的，每次
  刷新都可能换；启动时间是固定的，而且后开的那条更可能是要找的。
